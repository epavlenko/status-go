...
.PHONY: build-storage fetch-storage clean-storage test-storage
.PHONY: storage-help
...
lint-panics: generate
	GOFLAGS=-tags='$(BUILD_TAGS),lint,use_logos_storage' \
...
lint:
	$(CGO_ENV) golangci-lint --build-tags '$(BUILD_TAGS) lint use_logos_storage' run ./...

lint-fix: generate
	golangci-lint --build-tags '$(BUILD_TAGS) lint use_logos_storage' run --fix ./...
