...
//go:generate bash -c "printf '%s\n%s\n\n' '//go:build !use_nwaku' '// +build !use_nwaku' > gowaku_mock_test.go && go tool mockgen -source=gowaku.go -destination=gowaku_mock_test.go.tmp -package=wakuv2 && cat gowaku_mock_test.go.tmp >> gowaku_mock_test.go && rm -f gowaku_mock_test.go.tmp"
