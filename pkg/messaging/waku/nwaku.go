...
func (w *Waku) OnNewEnvelope(env common2.Envelope) error {
...
func (w *Waku) OnNewEnvelopes(envelope common2.Envelope, msgType common2.MessageType, processImmediately bool) error {
	if envelope == nil {
		return nil
	}
...
func convertPeersDataToPeerStats(peersData bindingscommon.PeersData) types2.PeerStats {
	result := make(types2.PeerStats)
	for peerID, peerInfo := range peersData {
		result[peerID] = types2.WakuV2Peer(peerInfo)
	}
	return result
}

func FormatPeerStats(wakuNode *waku.WakuNode) (types2.PeerStats, error) {
